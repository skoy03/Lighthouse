...
def dofetch(id, key, region):
    ...
    for i in range (len(s3)):
        ...
        Used = round(s4['TrafficUsed']/GB,1)
        percentageUsed = round((Used / TrafficPackageTotal) * 100,1)
        remainingpercentage = round((TrafficPackageTotal - Used) / TrafficPackageTotal * 100,1)
        ...
        if TrafficUsed < GB:
            useds = f"{round(TrafficUsed / MB,1)}MB"
        elif TrafficUsed < TB:
            useds = f"{round(TrafficUsed / GB,1)}GB"
        else:
            useds = f"{round(TrafficUsed / TB,1)}TB"
                        
        if TrafficPackageRemaining < GB:
            remaining = f"{round(TrafficPackageRemaining / MB,1)}MB"
        elif TrafficPackageRemaining < TB:
            remaining = f"{round(TrafficPackageRemaining / GB,1)}GB"
        else:
            remaining = f"{round(TrafficPackageRemaining / TB,1)}TB"
